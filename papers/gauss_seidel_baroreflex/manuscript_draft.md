...
The swap experiment provides causal proof that the bias originates from the sequential coupling architecture. When module order was reversed, the bias direction also reversed: ΔMAP switched from +44.7 mmHg (heart→neuro) to ±0.0 mmHg (neuro→heart), as shown in Figure 3A. This reversal cannot be explained by parameter variation — it is a direct consequence of which module sees updated state and which sees the previous time step's state.
...
Under heart→neuro ordering, `heart.compute()` runs first: loop A computes HR/SVR targets from the current MAP and writes them into `self.heart_rate` and `self.SVR`. Then `neuro.compute()` sees the updated MAP and computes loop B's FactorCommands. Critically, loop B's SVR multiplicative command is applied *after* the current step's loop A has already set `self.SVR` — loop B's effect on SVR will not be seen by loop A until the next step. The SVR accumulation that loop A targets therefore starts from a lower base in this ordering, producing a conservative MAP trajectory.

Under neuro→heart ordering (Figure 3B), `neuro.compute()` runs first using the stale MAP from the previous step. It emits FactorCommands — including a multiplicative SVR increase (`net_SVR_mult = 1.0 + seizure_effect * 0.3`, neuro.py lines 231–233) — that are applied directly to `self.SVR` before `heart.compute()` runs. When `heart.compute()` then executes, loop A's SVR target `SVR_increase = 1.0 + 2.0 * self.sympathetic * max(0.0, error)` (heart.py line 183) uses the *already-elevated* SVR as its starting point. This creates a SVR feedback loop: neuro's FactorCommands push SVR upward, loop A targets the new higher SVR, the next neuro step sees the updated MAP (now somewhat restored) and may emit further SVR increases, and the cycle reinforces. This SVR accumulation drives MAP from 100 toward 144.7 mmHg under neuro→heart ordering (Figure 3B).

Under heart→neuro ordering, the same FactorCommands are emitted after loop A has completed — they overwrite the SVR that loop A set, but loop A does not get to react to this modification until the following step. The ordering therefore determines whether loop A's SVR targets are computed on the baseline or the neuro-modified SVR — a difference that, compounded over 60 steps at dt = 0.01 s, produces the +44.7 mmHg equilibrium divergence.

The asymmetry reverses during hemorrhage because the dominant physics changes: blood volume loss drives MAP downward, and loop B's SVR-increasing FactorCommands become compensatory rather than destabilizing. When MAP is falling due to volume loss, neuro's stale-MAP computation in the neuro→heart ordering triggers a large SVR increase command, which (as shown above) causes an overshoot in the SVR accumulation trajectory — and hence in MAP recovery. This is why neuro→heart produces MAP = 89.0 mmHg at t = 30 s (undershooting relative to the reference), while heart→neuro's conservative SVR targeting produces the more accurate recovery of MAP = 98.4 mmHg. The mechanism is the same (SVR accumulation via the ordering of FactorCommand application relative to loop A's SVR target computation), but its effect on the MAP trajectory reverses depending on whether MAP is rising or falling.
