...
sol = solve_ivp(lambda t, y: vc._unified_rhs(t, y), (0.0, T_END), y0,
                method='DOP853', rtol=1e-6, atol=1e-8, max_step=0.5, t_eval=t_eval)
print(f'Done in {time.time()-t0:.1f}s, nfev={sol.nfev}, success={sol.success}')
dp_map = []
for i in range(sol.y.shape[1]):
    vc._cached_inputs.clear()
    vc._unpack_unified_state(sol.y[:, i])
    dp_map.append(float(vc.heart.mean_arterial_pressure))
dp_map = np.array(dp_map)
print(f'DOP853: [{dp_map.min():.4f}, {dp_map.max():.4f}], final={dp_map[-1]:.4f}')

with open('experiments/ref_dop853.json', 'w') as f:
    json.dump({'t': t_eval.tolist(), 'map': dp_map.tolist(), 'nfev': int(sol.nfev)}, f)
print('Saved: experiments/ref_dop853.json')