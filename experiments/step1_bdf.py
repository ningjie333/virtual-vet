...
sol = solve_ivp(lambda t, y: vc._unified_rhs(t, y), (0.0, T_END), y0,
                method='BDF', rtol=1e-6, atol=1e-8, max_step=1.0, t_eval=t_eval)
print(f'Done in {time.time()-t0:.1f}s, nfev={sol.nfev}, success={sol.success}')
bdf_map = []
for i in range(sol.y.shape[1]):
    vc._cached_inputs.clear()
    vc._unpack_unified_state(sol.y[:, i])
    bdf_map.append(float(vc.heart.mean_arterial_pressure))
bdf_map = np.array(bdf_map)
print(f'BDF: [{bdf_map.min():.4f}, {bdf_map.max():.4f}], final={bdf_map[-1]:.4f}')

with open('experiments/ref_bdf.json', 'w') as f:
    json.dump({'t': t_eval.tolist(), 'map': bdf_map.tolist(), 'nfev': int(sol.nfev)}, f)
print('Saved: experiments/ref_bdf.json')