"""Subprocess test: 完全隔离的独立进程验证 neuro→heart MAP"""
import subprocess, os, textwrap

# Get correct src path - pwd is already the project root (virtual-vet/)
pwd = os.getcwd()
...
for i in range(n_steps):
    vc.step()
print(f"HR={{vc.heart.heart_rate:.2f}}, MAP={{vc.heart.mean_arterial_pressure:.3f}}", flush=True)
"""
...
heart_script = SCRIPT.format(src_dir=src_dir, class_def=heart_class)
neuro_script = SCRIPT.format(src_dir=src_dir, class_def=textwrap.dedent(neuro_class))

print('\n=== Subprocess Test (完全隔离的独立进程) ===\n')

r1 = subprocess.run(['python', '-c', heart_script], capture_output=True, text=True)
print('heart-first:')
...
r2 = subprocess.run(['python', '-c', neuro_script], capture_output=True, text=True)
print('neuro-first:')
...
if '100' in out2 and '144' not in out2:
    print('結論: neuro-first MAP ≈ 100.0（與順序相關）')
elif '144' in out2:
    print('結論: neuro-first MAP ≈ 144.7（與順序無關）')
else:
    print(f'無法判定，輸出: {out2}')