"""Quick convergence check with key dt values."""
...
# Reference
print("=== Reference (Radau rtol=1e-5) ===")
vc = _make_vc()
...
print(f"  Reference: {len(t_dense)} points, min_MAP={ref_map.min():.1f}")
...
for dt in DT_GRID:
    ...
    if len(common_t) > 0:
        ...
        map_devs = np.abs(f(common_t) - ref_map[ref_mask])
        rmse_map = float(np.sqrt(np.mean(map_devs**2)))
    pure_results[dt] = rmse_map
    print(f"  dt={dt:.3f} t_end={vc.current_time_s:.2f}s RMSE={rmse_map:.2f} elapsed={elapsed:.1f}s")
...
for dt in DT_GRID:
    ...
    save_interval = max(1, int(DT_SAVE / dt))
    ts = []
    for step_i in range(int(T_END / dt)):
        vc.step()
        if step_i % save_interval == 0:
            ts.append(_record(vc, vc.current_time_s))
    elapsed = time.perf_counter() - t0
    ...
    if len(common_t) > 0:
        ...
        map_devs = np.abs(f(common_t) - ref_map[ref_mask])
        rmse_map = float(np.sqrt(np.mean(map_devs**2)))
    print(f"  dt={dt:.3f} RMSE={rmse_map:.2f} elapsed={elapsed:.1f}s")

print("\n=== Summary ===")
print(f"{'dt':>8} {'Pure Euler RMSE':>16} {'Seq Euler RMSE':>16}")
for dt in DT_GRID:
    print(f"{dt:>8.3f} {pure_results[dt]:>16.2f}")