"""Test explicit RK45 on tiny interval"""
...
# Try RK45 (explicit, 0→1s)
print('Testing RK45 on (0→1s)...')
t0 = time.time()
sol = solve_ivp(lambda t, y: vc._unified_rhs(t, y), (0.0, 1.0), y0,
               method='RK45', rtol=1e-6, atol=1e-8,
               max_step=0.1)
t1 = time.time()
print(f'RK45(0→1s): {t1-t0:.3f}s, nfev={sol.nfev}, success={sol.success}')
if sol.success:
    print(f'  y(1)[:3] = {sol.y[:,-1][:3]}')
