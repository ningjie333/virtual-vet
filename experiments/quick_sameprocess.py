"""Quick test: both orderings in same process"""
...
vc1 = VC_HeartFirst(body_weight_kg=20.0)
vc1.dt = DT
for i in range(n_steps):
    vc1.step()
    if vc1.lifecycle.is_dead(): break
print(f'Heart-first: HR={vc1.heart.heart_rate:.2f}, MAP={vc1.heart.mean_arterial_pressure:.3f}')

vc2 = VC_NeuroFirst(body_weight_kg=20.0)
vc2.dt = DT
vc2._cached_inputs.clear()
for i in range(n_steps):
    vc2.step()
    if vc2.lifecycle.is_dead(): break
print(f'Neuro-first: HR={vc2.heart.heart_rate:.2f}, MAP={vc2.heart.mean_arterial_pressure:.3f}')