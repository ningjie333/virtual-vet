...
for i in range(sol.y.shape[1]):
    ...
    rd_map.append(float(vc.heart.mean_arterial_pressure))
rd_map = np.array(rd_map)
print(f'Radau: [{rd_map.min():.4f}, {rd_map.max():.4f}], final={rd_map[-1]:.4f}')

with open('experiments/ref_radau.json', 'w') as f:
    json.dump({'t': t_eval.tolist(), 'map': rd_map.tolist(), 'nfev': int(sol.nfev)}, f)
print('Saved: experiments/ref_radau.json')