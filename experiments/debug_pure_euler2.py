"""Debug: trace Pure Euler through blood loss onset (t=5s+)."""
import sys, os, types
import numpy as np
...
dt = 0.01
total_steps = 900  # up to t=9s

print("Tracing from t=0 to t=9s through blood loss onset...")
for step_i in range(total_steps):
    ...
    if step_i % 50 == 0:
        ...
        print(f"step={step_i:4d} t={vc.current_time_s:.2f}s HR={hr:.1f} SV={sv:.3f} BV={bv:.1f} MAP={map_v:.1f} CO={co:.1f} | dydt={dydt[:4]} | y[:4]={y[:4]}")
        if has_nan:
            print(f"  NaN detected! y_before={y_before[:4]}")
            break
